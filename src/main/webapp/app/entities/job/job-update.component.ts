      ...
      if (this.job.id) {
        this.jobService()
          ...
          .then(param => {
            this.isSaving = false;
            this.previousState();
            this.alertService.showInfo(this.t$('turkishrentalApp.job.updated', { param: param.id }));
          })
          .catch(error => {
            this.isSaving = false;
        ...
        this.jobService()
          ...
          .then(param => {
            this.isSaving = false;
            this.previousState();
            this.alertService.showSuccess(this.t$('turkishrentalApp.job.created', { param: param.id }).toString());
          })
          .catch(error => {
            this.isSaving = false;
            this.alertService.showHttpError(error.response);
